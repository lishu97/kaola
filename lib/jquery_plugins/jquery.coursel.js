...
(function($){
	function Carousel(options) {
		...
		this.imgWidth = options.imgWidth || options.width;
	...
	Carousel.prototype = {
		...
		init : function(){ // 动态创建所需要使用到的DOM结构
			...
			for (var i = 0, len = this.imgs.length; i < len; i++) {
				...
				// 创建 li 元素
				var _li = $(`<li class='img'><a href="javascript:void(0)"><img src="${_img.src}"></a></li>`);
				// 将 _li 添加到 ul 中
				_li.appendTo(_ul);
				if (this.type === "fade") { // 淡入淡入式轮播				
					_li.css({
						position:"absolute",
						top:0,
						left:-(this.imgWidth - $(window).width())/2,
						display:"none"
					});
					if (i === 0)
						_li.show();
		...
		fade : function(){
			// 当前图片淡出，即将显示图片淡入
			var imgs = $(".imgs .img",$(this.target))
			imgs.eq(this.currentIndex).stop().fadeOut();
			imgs.eq(this.nextIndex).stop().fadeIn();

			// 小圆点
			$(".circles span",$(this.target)).attr("class","");
			$(".circles span",$(this.target)).eq(this.nextIndex).attr("class","current");
